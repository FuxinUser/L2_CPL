...
namespace LabelPrint.Actor
{
    public class LprSndEdit : BaseActor
    {
        ...
        public LprSndEdit(ISysAkkaManager akkaManager, ISysController sysController, ILog log) : base(log)
        {
            ...
            //_sysController.SaveAPStatusToL25("SystemAP_12", "1");
        ...
        private void Handle_MQ_Message(MQPool.MQMessage msg)
        {
            ...
            if (msg.ID == InfoLpr.SampleCut.Event)
            {
                var info = msg.Data as SampleInfo;
                var coilID = info.CoilID;
                var stNo = info.StNo;
                var thick = Math.Round(info.Thick, 3).ToString();
                var sampleNo = info.SampleNo;
                var samplePos = info.SamplePos;
                _log.I("列印鋼捲標籤", $"列印試樣鋼捲{coilID}標籤,StNo={stNo},Thick={thick},SampleNo={sampleNo},SamplePos={samplePos}");

                var zcmd = new ZebraCommand();
                zcmd.ZPL = coilID.zplQRCmd(stNo, thick, sampleNo, samplePos);
                _sndActor.Tell(zcmd);
            }
        }
        ...
        {
            var zplCmd = new ZplInstructionUtil();
            var zpl = "";
