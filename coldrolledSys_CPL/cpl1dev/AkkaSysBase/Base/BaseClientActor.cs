...
namespace AkkaSysBase.Base
{
    public class BaseClientActor : BaseActor
    {
        ...
        {
            //_log.I("TCP接收資料", "Handle_Tcp_Received. message=" + msg);
            //_log.I("TCP接收資料", "ByteString=" + msg.Data.ToArray());
        ...
        protected virtual void TcpCommandFailed(Tcp.CommandFailed message)
        {
            ...
            _log.E("TCP操作失敗", $" Thread ID=>{Thread.CurrentThread.ManagedThreadId}");
        ...
        {
            _log.I("TCP連線關閉", " Message.IsPeerClosed=" + message.Event);
            //_tcpStatuts = TCPDef.Statuts.Closed;
        }
        ...
        protected virtual void DeQueuSnd()
        {
        ...
        protected virtual void ReDeQueuSnd()
        {          
            //  若 queue 沒資料了就結束
            if (_sndQueu.Count <= 0)
                return;
