...
namespace CPL1HMI
{
    public class ShiftHandler
    {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    if (dc.ColumnName.Equals(nameof(WorkScheduleEntity.TBL_WorkSchedule.Team)))
                    {
                        ...
                        var control = PublicForms.CrewMaintainance.Pnl_Group.Controls.Find(controlName, true).FirstOrDefault();
                        if (string.IsNullOrEmpty(control.Text.Trim()))
                        {
                            ...
                            {
                                control.Text = dr[dc].ToString().Trim();//不转换甲乙丙丁,直接显示ABCD //Fun_TeamChangeWord(dr[dc].ToString());
                            }
                        }
                        else
                        {
                            ...
                            //control.Text = control.Text.Trim() + "  |  " + dr[dc].ToString().Trim();//不转换甲乙丙丁,直接显示ABCD //Fun_TeamChangeWord(dr[dc].ToString()); 
                            //if (control.Text == "  |  ") { control.Text = string.Empty; }
                        }
                        //((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find("Lbl_" + intDay + "_" + strShift, false)[0])).Text = TeamWordChange(dr[dc].ToString());
                    }
                }
            }
        }
        ...
        {
            string strSql = Frm_5_4_SqlFactory.Frm_5_4_DeleteOldWorkSchedule(ShiftDate);
            try
            {
                DataAccess.GetInstance().Fun_ExecuteQuery(strSql, "删除排班");
            ...
            {
                //DialogHandler.Instance.Fun_DialogShowOk($"删除排班失败:{ex}", "删除排班", 3);
        ...
        public bool Fun_InsertShift(DataTable dt)
        {
            ...
            string strInsElem_New = Frm_5_4_SqlFactory.Fun_GetInsertSqlFromDataTable(dt, nameof(WorkScheduleEntity.TBL_WorkSchedule));

            try
            {
                DataAccess.GetInstance().Fun_ExecuteQuery(strInsElem_New, "新增排班");
            ...
            {
                //DialogHandler.Instance.Fun_DialogShowOk($"新增排班失败:{ex}", "新增排班", 3);
        ...
        public void Fun_SettingDayOfWeek(int Year, int Month)
        {
            ...
            {
                ((Label)PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_Week_{days + 1}", false)[0]).Text = Fun_WeekEnToZH(Date.AddDays(days).DayOfWeek.ToString());

                ((Label)PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_Day_{days + 1}", false)[0]).Text = (days + 1).ToString();
        ...
        {
            ((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_Day_{intLbl }", false)[0])).Visible = bolShow;//日期:1
            ((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_Week_{intLbl}", false)[0])).Visible = bolShow;//星期
            ((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_{intLbl }_N", false)[0])).Visible = bolShow;//夜
            ((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_{intLbl }_M", false)[0])).Visible = bolShow;//早
            ((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_{intLbl }_A", false)[0])).Visible = bolShow;//中
            ((Label)(PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_{intLbl}_R", false)[0])).Visible = bolShow;//休
        ...
        private void Fun_ClearDayAndWeek()
        {
            ...
            {
                ((Label)PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_Week_{days + 1}", false)[0]).Text = string.Empty;
                ((Label)PublicForms.CrewMaintainance.Pnl_Group.Controls.Find($"Lbl_Day_{days + 1}", false)[0]).Text = string.Empty;
            }
        }
        ...
        private string Fun_WeekEnToZH(string Week)
        {
            ...
            switch (Week)
            {
                ...
                case "Tuesday": Week = userUICulture.Equals("en") ? "Tues" : "星期二"; break;
                case "Wednesday": Week = userUICulture.Equals("en") ? "Wed" : "星期三"; break;
                case "Thursday": Week = userUICulture.Equals("en") ? "Thur" : "星期四"; break;
        ...
        //    PublicForms.CrewMaintainance.Lbl_NowMonth.Text = PublicForms.CrewMaintainance.Dtp_SatrtDate.Value.ToString("yyyy 年 MM 月");

        //    //PublicForms.CrewMaintainance.Lbl_Mode_Show.Text = $"排班原则:{getMode}";
        //}
        //public void Fun_ShiftTimeDisplay()
        //{
        //    PublicForms.CrewMaintainance.Lbl_Night.Text =
        //        PublicForms.CrewMaintainance.Lbl_Night_1.Text =
        //        $"{Night_Start}-{Night_End}";

        //    PublicForms.CrewMaintainance.Lbl_Morning.Text =
        //        PublicForms.CrewMaintainance.Lbl_Morning_1.Text =
        //        $"{Morning_Start}-{Morning_End}";

        //    PublicForms.CrewMaintainance.Lbl_Afternoon.Text =
        //        PublicForms.CrewMaintainance.Lbl_Afternoon_1.Text =
        ...
        //    Afternoon_End = string.Empty;
        //}
        //private string TeamWordChange(string team)
        //{
        //    string changeWord = "R";
        //    switch (team)
        //    {
