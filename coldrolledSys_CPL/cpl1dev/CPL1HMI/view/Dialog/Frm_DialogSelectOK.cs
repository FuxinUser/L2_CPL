...
namespace CPL1HMI
{
    public partial class Frm_DialogSelectOK : Frm_DialogBase
    {
        #region Color
        ...
        public void DialogShow(string strMessage, string strTitle, string strBtn1 = "", string strBtn2 = "", Image img = null, int intStatus = 0)
        {
            if (img == null)
            {
                switch (intStatus)
            ...
            //Lbl_Tile.Text = LanguageHelper.GetLanguageText(strTitle);
            //Lbl_ShowMessage.Text = LanguageHelper.GetLanguageText(strMessage);
            Pic_Type.BackgroundImage = img;

            Txt_ShowMessage.Select(0, 0);
            Lbl_Tile.Focus();

            if (!string.IsNullOrEmpty(strBtn1))
            {
                Btn_Yes.Text = strBtn1;
            }

            if (!string.IsNullOrEmpty(strBtn2))
            {
                Btn_No.Text = strBtn2;
            }
        }
