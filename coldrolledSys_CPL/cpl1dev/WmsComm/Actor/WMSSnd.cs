...
namespace WMSComm.Actor
{
    public class WMSSnd : BaseClientActor
    {
        ...
        public WMSSnd(ISysAkkaManager akkaManager, ISysController sysController, AkkaSysIP akkaSysIp, ILog log, AggregateService agService) : base(akkaSysIp, log)
        {
        ...
        protected override void TcpReceivedData(Tcp.Received msg)
        {
            ...
            //只需判斷是否為ACK報文
            var ackMsg = MsgAnalUtil.RawDeserialize(msg.Data.ToArray(), typeof(WMS_ACK_Structure)) as WMS_ACK_Structure;
            ...
            {
                _log.E("接收應答電文", $"對方回絕電文，CODE:{ackMsg.ReceivedMsgID.ToStr()} 原因:{ackMsg.ErrorMsg.ToStr()}");
                if(_connection!=null)ReDeQueuSnd();
                return;
            }

            //
            if (ackMsg.ReceivedMsgID.ToStr().Equals(WMSSysDef.RcvMsgCode.HeartBeatCode))
                return;



            _log.D("接收應答電文", $"應答電文 CODE:{ackMsg.ReceivedMsgID.ToStr()} 原因:{ackMsg.ErrorMsg.ToStr()}");
        ...
        private void ProEventCmd(EventDef.CMDSET msg)
        {
            switch (msg)
            {
                ...
                case CMDSET.DETECT_SND_QUEU:
                    if (_connection != null) DeQueuSnd();
                    break;
                case EventDef.CMDSET.SND_HEARTBEAT:
                    if(_connection!=null) SndHeatbeat();
