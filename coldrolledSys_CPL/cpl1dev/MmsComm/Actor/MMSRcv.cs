...
namespace MMSComm.Actor
{
    public class MMSRcv : BaseServerActor
    {
        ...
        public MMSRcv(ISysAkkaManager akkaManager, ISysController sysController, AkkaSysIP akkaSysIp, ILog log, AggregateService agService, IMsgProController msgProService) : base(akkaSysIp, log)
        {
        ...
        public class TCPRcvDataVaildActor : BaseActor
        {
            ...
            private int _analysisFail;
            //Byte暫存器
            //private ByteString _byteKeeper = ByteString.Empty;
            ...
            private void TcpReceivedData(Tcp.Received msg)
            {
                ...
                //_log.Debug("TCP接收資料", "ByteString=" + msg.Data.ToString() + "Count=" + msg.Data.Count.ToString());
            ...
            private void ProTCPRcvData(Tcp.Received msg)
            {
                ...
                {
                    byte b = 0x0A;
                    var endTagIndex = Array.IndexOf(_listByte.ToArray(), b);
            ...
            private void SendAckMsg(string msgID, string isAccept)
            {
                ...
                var comMsg = new CommonMsg(length: bytes.Length.ToString(), id: ackMsg.Header.Code.ToStr(), bytes: bytes, isAck:true);
                _msgProService.CreateMMSWMSMsg("TBL_MMS_ReceiveRecord", comMsg);
